...
| 2026-07-19 | Incidental (found in T-13): `Player` exposes no public handler-registration API — `registerHandler` lives only on the private `_sourceManager`, so consumers can't add a custom `ISourceHandler`. AGENTS.md documents `registerHandler` as if reachable. | OPEN | Not in any current task. Candidate: add `Player.registerHandler` (delegates to `_sourceManager`) in a future task or fold into T-25 API review. |
