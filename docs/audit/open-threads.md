...
- **Source handlers section** (T-14/F-37): document `Player.registerHandler(handler)` (prepended = highest priority) and the `ISourceHandler` lifecycle — constructed once, `prepare` per load, `reset?()` between loads, `dispose()` once on player dispose. This is the API the audit flagged as missing from the README.
