...
| T-04 | Web Audio routing policy + CORS rework | DONE | 05a6f0c | 2026-07-18 | DECISION (user, overrides amendment step1): explicit `webAudioRouting:'always'\|'never'` (default always), NO usage heuristic. 'always'=route+crossOrigin for cross-origin html5 (EQ/viz/fades work out of box); 'never'=no graph, no crossOrigin, no AudioContext (guarded in setupAudioGraph + play + initialize; StrategyInitOptions.audioContext now optional). `corsFallback:false` opt-in: one retry w/o crossOrigin+graph on media error, respects signal. Shared isCrossOrigin util (utils/url.ts) reused by html5 strategy + player. +5 vitest + e2e routing.spec (all 3 engines) — WebKit now plays plain html5 via 'never' (10 e2e pass +1 skip). DROPPED amendment's "no crossOrigin unless feature used" criterion (unachievable: features requested post-load, decision irreversible, lazy attach forbidden). |
| T-05 | HLS runtime errors + recovery | DONE | (see follow-up) | 2026-07-18 | HLSHandler: persistent ERROR listener survives readiness resolve; pre-ready fatal fails load (as before), post-ready fatal → recovery. Fatal NETWORK: 3 startLoad() retries w/ 1s/2s/4s backoff, abort-aware via retained signal; FRAG_BUFFERED resets budget. Fatal MEDIA: recoverMediaError, then swapAudioCodec+recover, then give up. Give-up → detachMedia+destroy + surface via new SourceCapabilities.onRuntimeError → Player.handleRuntimeError (emit once + transition error). Dedupe: element error handler skipped when handler exposes onRuntimeError. HlsInstance +startLoad/stopLoad/recoverMediaError/swapAudioCodec; cleanup clears backoff timer. +4 tests (fake timers). e2e green. |
