...
| T-04 | Web Audio routing policy + CORS rework | DONE | (see follow-up) | 2026-07-18 | DECISION (user, overrides amendment step1): explicit `webAudioRouting:'always'\|'never'` (default always), NO usage heuristic. 'always'=route+crossOrigin for cross-origin html5 (EQ/viz/fades work out of box); 'never'=no graph, no crossOrigin, no AudioContext (guarded in setupAudioGraph + play + initialize; StrategyInitOptions.audioContext now optional). `corsFallback:false` opt-in: one retry w/o crossOrigin+graph on media error, respects signal. Shared isCrossOrigin util (utils/url.ts) reused by html5 strategy + player. +5 vitest + e2e routing.spec (all 3 engines) — WebKit now plays plain html5 via 'never' (10 e2e pass +1 skip). DROPPED amendment's "no crossOrigin unless feature used" criterion (unachievable: features requested post-load, decision irreversible, lazy attach forbidden). |
